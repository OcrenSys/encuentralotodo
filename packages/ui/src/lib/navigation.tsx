...
export function TopSearchHeader({ children }: { children: ReactNode }) {
  return (
    <div className="sticky top-0 z-30 bg-[var(--color-background)]/88 px-4 pb-3 pt-4 backdrop-blur-lg sm:px-6 lg:px-8 xl:px-10">
      {children}
    </div>
  );
}
...
export function BottomNavigation({
  ...
  current?: 'home' | 'search' | 'favorites' | 'profile';
}) {
  const items = [
  ...
  return (
    <div className="fixed bottom-3 left-1/2 z-40 w-[calc(100%-24px)] max-w-md -translate-x-1/2 rounded-full border border-white/70 bg-white/92 p-2 shadow-[0_20px_48px_rgba(17,39,60,0.14)] backdrop-blur-xl lg:hidden">
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-x-0 bottom-0 z-50 rounded-t-[32px] bg-white px-5 pb-8 pt-5 shadow-2xl focus:outline-none sm:left-auto sm:right-6 sm:top-6 sm:w-[420px] sm:rounded-[32px]">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <Dialog.Description className="text-sm text-[var(--color-text-muted)]">
                Ajusta categoría, distancia y promociones activas.
              </Dialog.Description>
            </div>
          ...
          <Card className="border-none bg-[var(--color-background)] shadow-none">
            {content}
          </Card>
        </Dialog.Content>
      </Dialog.Portal>

...
function resolveFirebaseTokenVerifier(config: FirebaseAuthProviderConfig = {}) {
  ...
  const appName =
    config.appName ?? `encuentralotodo-auth-${projectId ?? 'default'}`;
...
export class MockAuthProvider implements AuthProvider {
  ...
  constructor(
    private readonly users: Record<string, MockAuthUser> = defaultMockUsers,
  ) {}
    ...
    const user =
      ...
      Object.values(this.users).find(
        (candidate) =>
          ...
          candidate.email.toLowerCase() === normalizedToken,
      );

    if (!user) {
      throw new AuthTokenVerificationError(
        'Mock auth token was not recognized.',
      );
    }
...
export class FirebaseAuthProvider implements AuthProvider {
  ...
  private readonly tokenVerifier: ReturnType<
    typeof resolveFirebaseTokenVerifier
  >;

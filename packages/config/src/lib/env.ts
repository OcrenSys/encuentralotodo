...
const serverEnvSchema = z
  ...
  .superRefine((env, ctx) => {
    if (
      (env.DATA_MODE === 'prisma' || env.AUTH_PROVIDER !== 'mock') &&
      !env.DATABASE_URL
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'DATABASE_URL is required when DATA_MODE=prisma or AUTH_PROVIDER is not mock.',
        path: ['DATABASE_URL'],
      });
    }
    ...
    if (env.AUTH_PROVIDER !== 'mock' && env.DATA_MODE !== 'prisma') {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'Real auth providers require DATA_MODE=prisma so local users can be resolved in the database.',
        path: ['DATA_MODE'],
      });
    }
    ...
    if (!env.FIREBASE_SERVICE_ACCOUNT_JSON) {
      const hasAnyExplicitCredential = Boolean(
      ...
      if (hasAnyExplicitCredential && !env.FIREBASE_PROJECT_ID) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'FIREBASE_PROJECT_ID is required when using explicit Firebase Admin credentials.',
      ...
      if (hasAnyExplicitCredential && !env.FIREBASE_CLIENT_EMAIL) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'FIREBASE_CLIENT_EMAIL is required when using explicit Firebase Admin credentials.',
      ...
      if (hasAnyExplicitCredential && !env.FIREBASE_PRIVATE_KEY) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'FIREBASE_PRIVATE_KEY is required when using explicit Firebase Admin credentials.',
...
const publicEnvSchema = z
  .object({
    ...
    NEXT_PUBLIC_AUTH_PROVIDER: z
      .enum(['mock', 'firebase', 'cognito'])
      .default('mock'),
    NEXT_PUBLIC_FILE_STORAGE_PROVIDER: z
      .enum(['firebase', 's3'])
      .default('firebase'),
  ...
  .superRefine((env, ctx) => {
    if (
      ...
      env.NEXT_PUBLIC_FILE_STORAGE_PROVIDER !== 'firebase'
    ) {
      return;
    }

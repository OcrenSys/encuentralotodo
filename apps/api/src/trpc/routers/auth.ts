...
import {
  removeOwnBusinessRoleInputSchema,
...
export const authRouter = router({
  ...
  profile: protectedProcedure.query(({ ctx }) =>
    ctx.userAdminService.getSelfProfile(),
  ),
  updateProfile: protectedProcedure
    .input(selfProfileUpdateInputSchema)
    .mutation(({ ctx, input }) => {
      return ctx.userAdminService.updateSelfProfile(input);
    }),
  removeOwnBusinessRole: protectedProcedure
    .input(removeOwnBusinessRoleInputSchema)
    .mutation(({ ctx, input }) => {
    ...
    .mutation(({ ctx, input }) => {
      if (ctx.env.AUTH_PROVIDER !== 'mock') {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message:
            'Use the active auth provider client SDK to sign in and send its ID token to the backend.',
        });
      }

...
class InMemoryAuthIdentityRepository implements AuthIdentityRepositoryPort {
  ...
  async findCurrentUserByIdentity(
  ...
  async createUserFromIdentity(identity: VerifiedIdentity) {
    const user: LocalUserRecord = {
      ...
      email:
        identity.email ??
        `${identity.externalUserId}@auth.encuentralotodo.local`,
      role: 'UNASSIGNED',
...
describe('AuthIdentityService', () => {
  ...
  it('reuses and refreshes an existing local user on subsequent requests', async () => {
    ...
    expect(secondUser.avatarUrl).toBe(
      'https://cdn.encuentralotodo.app/avatars/ana-v2.png',
    );
  ...
  it('preserves an existing superadmin and avoids creating a duplicate user during login resolution', async () => {
    const repository = {
      findCurrentUserByIdentity: jest.fn(
        async (_provider, _externalUserId) => null,
      ),
      ...
      createUserFromIdentity: jest.fn(async () => {
        throw new Error(
          'createUserFromIdentity should not be called for existing privileged users',
        );
      }),
      upsertIdentityForUser: jest.fn(
        async (_userId: string, _identity: VerifiedIdentity) =>
          createCurrentUser({
            id: 'user-existing-superadmin',
            fullName: 'Jairo Martinez',
            email: 'ocrensys@gmail.com',
    ...
    expect(repository.upsertIdentityForUser).toHaveBeenCalledWith(

...
function createPlatformUser(
  overrides: Partial<RepositoryPlatformUserRecord> = {},
): RepositoryPlatformUserRecord {
  return {
    id: 'user-ana',
...
describe('UserAdminService', () => {
  ...
  it('rejects non-admin callers from searching platform users', async () => {
    ...
    await expect(
      service.searchUsers({ search: 'ana', limit: 10 }),
  ...
  it('rejects self role changes', async () => {
    ...
    await expect(
      service.updateUserRole({ userId: 'user-ana', role: 'ADMIN' }),
    ).rejects.toMatchObject({
      code: 'BAD_REQUEST',
  ...
  it('prevents removing the last active superadmin role', async () => {
    ...
    repository.findUserById.mockResolvedValue(
      createPlatformUser({ id: 'superadmin-2', role: 'SUPERADMIN' }),
    );
    repository.countUsersByRole.mockResolvedValue(1);

    await expect(
      service.updateUserRole({ userId: 'superadmin-2', role: 'ADMIN' }),
    ).rejects.toMatchObject({
      code: 'BAD_REQUEST',
  ...
  it('disables another account', async () => {
    const { service, repository } = createService();
    repository.findUserById.mockResolvedValue(
      createPlatformUser({ id: 'user-ana', role: 'USER' }),
    );
    repository.setUserActive.mockResolvedValue(
      createPlatformUser({ id: 'user-ana', isActive: false }),
    );

    const result = await service.setUserActive({
      userId: 'user-ana',
      isActive: false,
    });
  ...
  it('rejects disabling the caller account', async () => {
    ...
    repository.findUserById.mockResolvedValue(
      createPlatformUser({ id: 'superadmin-1', role: 'SUPERADMIN' }),
    );

    await expect(
      service.setUserActive({ userId: 'superadmin-1', isActive: false }),
    ).rejects.toMatchObject({
      code: 'BAD_REQUEST',

...
function mapUserBusinessAssignment(
  record: RepositoryUserBusinessRoleRecord,
): UserBusinessAssignment {
  return {
    id: record.id,
...
function mergeBusinessAssignments(
  ...
  return [...mappedAssignments, ...compatibilityAssignments].sort(
    (left, right) => {
...
function mapAuditLogEntry(record: RepositoryAuditLogRecord): AuditLogEntry {
  return {
    ...
    targetUser: record.targetUser
      ? mapPlatformUser(record.targetUser)
      : undefined,
  };
}
...
function supportsDirectUserFieldEditing(
  currentUser: RepositoryPlatformUserRecord | null,
) {
  if (!currentUser) {
    return false;
  }

  return currentUser.identities.every(
...
export class UserAdminService {
  ...
  ): Promise<ManagementListResult<PlatformUser>> {
    this.assertSuperAdmin();
  ...
  async getSelfProfile(): Promise<SelfProfile> {
    ...
    const [assignments, compatibilityOwnedBusinesses, auditLogs] =
      await Promise.all([
    ...
    return {
      ...
      verificationState: {
        hasVerifiedIdentity: user.identities.some(
  ...
  async updateSelfProfile(input: SelfProfileUpdateInput): Promise<SelfProfile> {
    ...
    if (!supportsDirectUserFieldEditing(existingUser)) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message:
          'This account profile is managed by the active auth provider and cannot be edited here yet.',
      });
    }

    const updatedUser = await this.dependencies.repository.updateUserProfile(
      currentUser.id,
      {
  ...
  async updateUserProfile(
    ...
    if (!supportsDirectUserFieldEditing(existingUser)) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message:
          'This account profile is managed by the active auth provider and only supports read-only details for now.',
      });
    }
  ...
  async updateBaseUserRole(
    input: UpdateBaseUserRoleInput,
  ): Promise<AdminUserDetail> {
    const actor = this.assertSuperAdmin();
    ...
    if (!isBaseRole(targetUser.role) && input.role === 'NO_ACCESS') {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message:
          'Platform admins must first be downgraded from their platform role before setting NO_ACCESS.',
      });
    }

    const updatedUser = await this.dependencies.repository.updateBaseUserRole(
      input.userId,
      input.role,
    );
    if (!updatedUser) {
  ...
  async updateUserRole(
    ...
    const updatedUser = await this.dependencies.repository.updateUserRole(
      input.userId,
      input.role,
    );
  ...
  async assignUserBusinessRole(
    input: AssignUserBusinessRoleInput,
  ): Promise<AdminUserDetail> {
    const actor = this.assertSuperAdmin();
    await Promise.all([
  ...
  async removeUserBusinessRole(
    input: RemoveUserBusinessRoleInput,
  ): Promise<AdminUserDetail> {
    const actor = this.assertSuperAdmin();
  ...
  async removeOwnBusinessRole(
    input: RemoveOwnBusinessRoleInput,
  ): Promise<SelfProfile> {
    const currentUser = this.assertAuthenticatedSelf();
    await this.dependencies.businessMembershipSyncService?.synchronizeAllBusinessMemberships();

    const assignments =
      await this.dependencies.repository.listUserBusinessRoles(currentUser.id);
    ...
    if (assignment.role !== 'MANAGER') {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message:
          'Only managers can remove themselves from a business assignment.',
      });
    }
  ...
  async transferBusinessOwnership(
    ...
    const ownerCount = await this.dependencies.repository.countBusinessOwners(
      input.businessId,
    );
  ...
  async setUserActive(
    input: SetPlatformUserActiveInput,
  ): Promise<PlatformUser> {
    const actor = this.assertSuperAdmin();
    const targetUser = await this.getExistingUser(input.userId);
    ...
    const updatedUser = await this.dependencies.repository.setUserActive(
      input.userId,
      input.isActive,
    );
  ...
  private async assertBusinessExists(
    businessId: string,
  ): Promise<RepositoryBusinessOptionRecord> {
    const businesses =
      await this.dependencies.repository.listBusinessesForAssignment();
  ...
  private async assertAnotherActiveSuperAdminExists() {
    const superAdminCount = await this.dependencies.repository.countUsersByRole(
      'SUPERADMIN',
      true,
    );

...
export interface UserAdminRepositoryPort {
  ...
  assignUserBusinessRole(input: {
    userId: string;
    businessId: string;
    role: BusinessAssignmentRole;
  }): Promise<RepositoryUserBusinessRoleRecord>;
  removeUserBusinessRole(input: {
    userId: string;
    businessId: string;
    role: BusinessAssignmentRole;
  }): Promise<void>;
  countBusinessOwners(businessId: string): Promise<number>;
  transferBusinessOwnership(input: {
    businessId: string;
    fromUserId: string;
    toUserId: string;
  }): Promise<void>;
...
function mapPlatformUserSearchRecord(
  record: any,
...
function mapUserBusinessRoleRecord(
  record: any,
...
function mapAuditLogRecord(record: any): RepositoryAuditLogRecord {
  return {
    ...
    actorUser: record.actorUser
      ? mapPlatformUserRecord(record.actorUser)
      : null,
    targetUser: record.targetUser
      ? mapPlatformUserRecord(record.targetUser)
      : null,
  };
}
...
export class UserAdminRepository implements UserAdminRepositoryPort {
  ...
  async assignUserBusinessRole(input: {
    userId: string;
    businessId: string;
    role: BusinessAssignmentRole;
  }) {
  ...
  async removeUserBusinessRole(input: {
    userId: string;
    businessId: string;
    role: BusinessAssignmentRole;
  }) {
  ...
  async transferBusinessOwnership(input: {
    businessId: string;
    fromUserId: string;
    toUserId: string;
  }) {
  ...
  private async setCanonicalBusinessRole(
    ...
    const duplicateIds = existingRecords
      ...
      .map(
        (existingRecord: (typeof existingRecords)[number]) => existingRecord.id,
      );

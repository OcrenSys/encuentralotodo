...
export class LeadService {
  ...
  private readonly promotionRepository: Pick<
  ...
  async updateLeadStatus(input: UpdateLeadStatusInput) {
    ...
    const updatedLead = await this.repository.updateStatus(
      input.leadId,
      input.status,
    );
    if (!updatedLead) {
  ...
  async updateLeadNotes(input: UpdateLeadNotesInput) {
    ...
    const updatedLead = await this.repository.updateNotes(
      input.leadId,
  ...
  private async assertLeadRelationsMatchBusiness(
    ...
    if (input.businessId !== businessId) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'Lead business context is invalid.',
      });
    }
    ...
    if (input.promotionId) {
      ...
      if (promotion.businessId !== businessId) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: 'Lead promotion context is invalid.',
        });
      }
    }
  }
  ...
  private ensureBusinessCanBeManaged(
    business:
      ...
      | RepositoryLeadWithBusinessRecord['business'],
  ) {
    if (!this.currentUser) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',

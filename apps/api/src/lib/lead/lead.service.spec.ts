import type { CreateLeadInput, UserProfile } from 'types';
...
} from '../promotion/promotion.repository';
import type {
  LeadRepositoryPort,
...
function createBusinessAccess(
  overrides: Partial<RepositoryBusinessAccessRecord> = {},
): RepositoryBusinessAccessRecord {
  return {
...
function createLeadRecord(
  overrides: Partial<RepositoryLeadRecord> = {},
): RepositoryLeadRecord {
  return {
...
function createLeadWithBusinessRecord(
  overrides: Partial<RepositoryLeadWithBusinessRecord> = {},
): RepositoryLeadWithBusinessRecord {
...
function createProductRecord(
  overrides: Partial<RepositoryProductRecord> = {},
): RepositoryProductRecord {
  return {
...
function createPromotionRecord(
  overrides: Partial<RepositoryPromotionRecord> = {},
): RepositoryPromotionRecord {
  return {
    ...
    description:
      overrides.description ?? 'Promocion activa para equipos corporativos.',
...
function createProductRepositoryMock(): jest.Mocked<
  Pick<ProductRepositoryPort, 'findById'>
> {
  return {
    findById: jest.fn(),
  };
...
function createPromotionRepositoryMock(): jest.Mocked<
  Pick<PromotionRepositoryPort, 'findById'>
> {
  return {
    findById: jest.fn(),
  };
...
describe('LeadService', () => {
  ...
  it('creates a WhatsApp lead with NEW status and meaningful summary', async () => {
    ...
    businessRepository.findBusinessAccessById.mockResolvedValue(
  ...
  it('creates a promotion lead resolving business and promotion context', async () => {
    const { service, repository, promotionRepository, businessRepository } =
      createService(null);
    const promotion = createPromotionRecord({
      id: 'promo-lunch',
      title: 'Lunch corporativo 2x1',
    });
    ...
    const result = await service.createLeadFromPromotionClick({
      promotionId: promotion.id,
    });
  ...
  it('creates a product lead resolving business and product context', async () => {
    const { service, repository, productRepository, businessRepository } =
      createService(null);
    ...
    const result = await service.createLeadFromProductClick({
      productId: product.id,
    });
  ...
  it('rejects an invalid product context for internal lead creation', async () => {
    const { service, businessRepository, productRepository } =
      createService(null);
    businessRepository.findBusinessAccessById.mockResolvedValue(
      createBusinessAccess(),
    );
    productRepository.findById.mockResolvedValue(
      createProductRecord({ businessId: 'biz-other' }),
    );
    ...
    await expect(
      ...
      code: 'BAD_REQUEST',
      message: 'Lead product context is invalid.',
    });
  });

  it('lists leads for an authorized owner', async () => {
    const { service, repository, businessRepository } =
      createService(ownerUser);
    businessRepository.findBusinessAccessById.mockResolvedValue(
      createBusinessAccess(),
    );
  ...
  it('rejects unauthorized lead inbox access', async () => {
    ...
    await expect(
      service.listByBusiness('biz-casa-norte'),
    ).rejects.toMatchObject({
      code: 'FORBIDDEN',
  ...
  it('allows a manager to update lead status', async () => {
    ...
    repository.findByIdWithBusiness.mockResolvedValue(
      createLeadWithBusinessRecord(),
    );
    repository.updateStatus.mockResolvedValue(
      createLeadRecord({ status: 'QUALIFIED' }),
    );
  ...
  it('persists notes updates and normalizes blank notes to undefined in the response', async () => {
    ...
    const result = await service.updateLeadNotes({
      leadId: 'lead-1',
      notes: '',
    });
  ...
  it('requires authentication for lead status changes', async () => {
    ...
    await expect(
      service.updateLeadStatus({ leadId: 'lead-1', status: 'CONTACTED' }),
    ).rejects.toMatchObject({
      code: 'UNAUTHORIZED',

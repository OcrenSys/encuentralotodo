...
export interface LeadRepositoryPort {
  ...
  findByIdWithBusiness(
    leadId: string,
  ): Promise<RepositoryLeadWithBusinessRecord | null>;
  create(input: CreateLeadInput): Promise<RepositoryLeadRecord>;
  updateStatus(
    leadId: string,
    status: LeadStatus,
  ): Promise<RepositoryLeadRecord | null>;
  updateNotes(
    leadId: string,
    notes: string | null,
  ): Promise<RepositoryLeadRecord | null>;
...
function mapLeadWithBusinessRecord(
  record: any,
...
export class LeadRepository implements LeadRepositoryPort {
  ...
  async create(input: CreateLeadInput) {
    const record = await this.prisma.$transaction(
      async (transaction: ReturnType<typeof getPrismaClient>) => {
  ...
  async updateStatus(leadId: string, status: LeadStatus) {
    const record = await this.prisma.lead
      .update({
        where: { id: leadId },
        data: { status },
        select: leadSelect,

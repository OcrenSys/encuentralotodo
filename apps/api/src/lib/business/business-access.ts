...
export function getBusinessMembershipRole(
  user: UserProfile | null | undefined,
  business: BusinessAccessIdentity,
) {
  if (!user) {
    return null;
  }

  const canonicalMembership = business.memberships?.find(
...
export function isBusinessManager(
  user: UserProfile | null | undefined,
  business: BusinessAccessIdentity,
) {
...
export function canAccessBusiness(
  ...
  return (
    ...
    isBusinessManager(user, business)
  );
}

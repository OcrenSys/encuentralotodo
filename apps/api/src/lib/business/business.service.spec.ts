...
function createBusinessRecord(
  overrides: Partial<RepositoryBusinessRecord> = {},
): RepositoryBusinessRecord {
  return {
    id: 'biz-casa-norte',
...
function createCurrentPlatformUser(
  overrides: Partial<ReturnType<typeof createCurrentUser>> = {},
) {
...
describe('BusinessService', () => {
    ...
    expect(result).toMatchObject({
      ...
      reviews: [
        expect.objectContaining({
          id: 'review-1',
          user: expect.objectContaining({ id: 'user-ana' }),
        }),
      ],
    });
  });
  ...
  it('listManagedBusinesses returns only businesses the authenticated owner can manage', async () => {
    ...
    const { service, repository, businessMembershipSyncService } =
      createService(actor);
    repository.listBusinessesByUserAccess.mockResolvedValue([
    ...
    expect(
      businessMembershipSyncService.synchronizeAllBusinessMemberships,
    ).toHaveBeenCalled();
    expect(repository.listBusinessesByUserAccess).toHaveBeenCalledWith(
      'owner-sofia',
      { includePending: true },
    );
  ...
  it('listManagedBusinesses returns the full management view for platform admins', async () => {
    ...
    repository.listBusinessesForManagement.mockResolvedValue([
      createBusinessRecord(),
    ]);
  ...
  it('allows the owner to update general business information', async () => {
    ...
    const input: UpdateBusinessInput = {
      ...
      description:
        'Descripción suficientemente larga para actualizar el negocio con datos revisados por el owner.',
      category: 'GENERAL_STORE',
      location: {
        lat: 18.49,
        lng: -69.93,
  ...
  it('allows an owner to change managers', async () => {
    ...
    const result = await service.updateBusiness({
      ...
      name: 'Casa Norte Market',
      description:
        'Descripción suficientemente larga para intentar cambiar managers sin permisos válidos.',
      category: 'GENERAL_STORE',
      location: {
        lat: 18.4861,
        lng: -69.9312,
  ...
  it('allows an owner to change the membership plan', async () => {
    ...
    const input: UpdateBusinessInput = {
      ...
      name: 'Casa Norte Market',
      description:
        'Descripción suficientemente larga para intentar cambiar el plan sin permisos válidos.',
      category: 'GENERAL_STORE',
      location: {
        lat: 18.4861,
        lng: -69.9312,
    ...
    repository.findUsersByIds.mockResolvedValue([managerUser]);
    repository.updateBusiness.mockResolvedValue(
      createBusinessRecord({ subscriptionType: 'FREE_TRIAL' }),
    );

    const result = await service.updateBusiness(input);

    expect(repository.updateBusiness).toHaveBeenCalledWith(
      expect.objectContaining({ subscriptionType: 'FREE_TRIAL' }),
    );
  ...
  it('allows a SuperAdmin to change the membership plan', async () => {
    ...
    const input: UpdateBusinessInput = {
      ...
      name: 'Casa Norte Market',
      description:
        'Descripción suficientemente larga para actualizar el plan desde un SuperAdmin.',
      category: 'GENERAL_STORE',
      location: {
        lat: 18.4861,
        lng: -69.9312,
  ...
  it('prevents managers from updating business records', async () => {
    ...
    const input: UpdateBusinessInput = {
      ...
      name: 'Casa Norte Market',
      description:
        'Descripción suficientemente larga para una actualización operativa desde manager.',
      category: 'GENERAL_STORE',
      location: {
        lat: 18.4861,
        lng: -69.9312,
  ...
  it('approve updates business status correctly', async () => {
    ...
    repository.approveBusiness.mockResolvedValue(
      createBusinessRecord({ status: 'APPROVED' }),
    );

    const result = await service.approveBusiness({
      businessId: 'biz-casa-norte',
    });
  ...
  it('create does not rely on client supplied owner identity as authoritative truth', async () => {
    ...
    expect(repository.createBusiness).toHaveBeenCalledWith(
      expect.objectContaining({ ownerId: 'owner-sofia' }),
    );
  });
  ...
  it('allows admins to create a business for another explicit owner', async () => {
    ...
    const result = await service.createBusinessForOwner({
      ...
      description:
        'Descripción suficientemente larga para el alta delegada desde administración.',
      category: 'GENERAL_STORE',
      location: {
        lat: 18.47,
        lng: -69.9,

...
export function resolveBusinessMembershipState(
  record: BusinessMembershipSourceRecord,
): ResolvedBusinessMembershipState {
  const legacyOwner = record.owner ?? undefined;
  const legacyManagersById = new Map<
    string,
    BusinessMembershipUserSourceRecord | undefined
  >(
  ...
  const membershipByUserId = new Map<
    string,
    ReturnType<typeof createMembershipRecord>
  >();
  ...
  for (const membership of record.userRoles ?? []) {
    ...
    if (existing && existing.role !== membership.role) {
      conflicts.push(
        `Business ${record.id} has multiple canonical roles for user ${membership.userId}.`,
      );
      continue;
    }
  ...
  if (!membershipByUserId.has(record.ownerId)) {
    membershipByUserId.set(
      record.ownerId,
      createMembershipRecord(record.ownerId, 'OWNER', legacyOwner),
    );
    missingCanonicalRoles.push({
    ...
    conflicts.push(
      `Business ${record.id} legacy owner ${record.ownerId} conflicts with canonical role ${membershipByUserId.get(record.ownerId)?.role}.`,
    );
  }
  ...
  for (const legacyManager of record.managers ?? []) {
    ...
    if (!existing) {
      membershipByUserId.set(
        ...
        createMembershipRecord(
          legacyManager.userId,
          'MANAGER',
          legacyManager.user ?? undefined,
        ),
      );
      missingCanonicalRoles.push({
        businessId: record.id,
        userId: legacyManager.userId,
        role: 'MANAGER',
      });
      continue;
    }
    ...
    if (existing.role !== 'MANAGER') {
      conflicts.push(
        `Business ${record.id} legacy manager ${legacyManager.userId} conflicts with canonical role ${existing.role}.`,
      );
    }
  }

  const memberships = Array.from(membershipByUserId.values()).sort(
    (left, right) => {
  ...
  if (owners.length !== 1) {
    conflicts.push(
      `Business ${record.id} must resolve to exactly one owner, found ${owners.length}.`,
    );
  }
...
export function collectCanonicalMembershipSync(
  records: BusinessMembershipSourceRecord[],
) {
  ...
  for (const record of records) {
    ...
    membershipState.missingCanonicalRoles.forEach((membership) => {
      missingRoleMap.set(
        `${membership.businessId}:${membership.userId}`,
        membership,
      );
    });

...
export interface BusinessRepositoryPort {
  ...
  ): Promise<RepositoryBusinessRecord[]>;
  listBusinessesForManagement(
    filters?: BusinessListFilters,
  ): Promise<RepositoryBusinessRecord[]>;
  listBusinessesByUserAccess(
    userId: string,
    filters?: BusinessListFilters,
  ): Promise<RepositoryBusinessRecord[]>;
  listBusinessesForManagementPage(
    filters: ListManagedBusinessesInput,
  ): Promise<RepositoryManagedBusinessesPage>;
  listBusinessesByUserAccessPage(
    userId: string,
    filters: ListManagedBusinessesInput,
  ): Promise<RepositoryManagedBusinessesPage>;
  findBusinessById(
    businessId: string,
  ): Promise<RepositoryBusinessRecord | null>;
  findBusinessAccessById(
    businessId: string,
  ): Promise<RepositoryBusinessAccessRecord | null>;
  ...
  updateBusiness(
    input: UpdateBusinessInput & {
      subscriptionType: RepositoryBusinessRecord['subscriptionType'];
      managers: string[];
    },
  ): Promise<RepositoryBusinessRecord | null>;
  approveBusiness(businessId: string): Promise<RepositoryBusinessRecord | null>;
  listBusinessMembershipSources(
    businessIds?: string[],
  ): Promise<BusinessMembershipSourceRecord[]>;
  upsertCanonicalMemberships(
    input: Array<{
      businessId: string;
      userId: string;
      role: BusinessAssignmentRole;
...
function mapBusinessRecord(record: any): RepositoryBusinessRecord {
  ...
  return {
    ...
    owner: membershipState.owner
      ? mapUserRecord(membershipState.owner)
      : undefined,
...
function buildBusinessWhere(
  filters: BusinessListFilters = {},
  accessClause?: Record<string, unknown>,
) {
...
function buildManagedBusinessWhere(
  filters: ListManagedBusinessesInput,
  accessClause?: Record<string, unknown>,
) {
  const clauses: Record<string, unknown>[] = [];
...
export class BusinessRepository implements BusinessRepositoryPort {
  ...
  async listBusinessesByUserAccess(
    userId: string,
    filters: BusinessListFilters = {},
  ) {
    const records = await this.prisma.business.findMany({
  ...
  async listBusinessesByUserAccessPage(
    userId: string,
    filters: ListManagedBusinessesInput,
  ) {
    const where = buildManagedBusinessWhere(filters, {
  ...
  async updateBusiness(
    ...
    const record = await this.prisma.$transaction(
      async (tx: ReturnType<typeof getPrismaClient>) => {
  ...
  async upsertCanonicalMemberships(
    input: Array<{
      businessId: string;
      userId: string;
      role: BusinessAssignmentRole;
    ...
    for (const membership of uniqueInput) {
      ...
      const recordToKeep =
        existingRecords.find(
          (existingRecord: (typeof existingRecords)[number]) =>
            existingRecord.role === membership.role,
        ) ?? existingRecords[0];
      const duplicateIds = existingRecords
        .filter(
          (existingRecord: (typeof existingRecords)[number]) =>
            existingRecord.id !== recordToKeep?.id,
        )
        .map(
          (existingRecord: (typeof existingRecords)[number]) =>
            existingRecord.id,

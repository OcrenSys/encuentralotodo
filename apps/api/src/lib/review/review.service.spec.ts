import type { CreateReviewInput, UserProfile } from 'types';
...
function createReviewRecord(
  overrides: Partial<RepositoryReviewRecord> = {},
): RepositoryReviewRecord {
  return {
    id: 'review-1',
    rating: 5,
...
function createReviewWithUserRecord(
  overrides: Partial<RepositoryReviewWithUserRecord> = {},
): RepositoryReviewWithUserRecord {
  return {
    ...createReviewRecord(),
    user: {
...
describe('ReviewService', () => {
  ...
  it('create review with valid data persists using the server-side current user', async () => {
    ...
    repository.create.mockResolvedValue(
      createReviewRecord({ userId: 'user-ana' }),
    );
  ...
  it('listByBusiness returns mapped Prisma-backed reviews with public user info', async () => {
    ...
    businessRepository.findBusinessById.mockResolvedValue(

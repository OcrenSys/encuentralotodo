...
function mapUser(
  record: NonNullable<RepositoryReviewWithUserRecord['user']>,
): UserProfile {
  return {
    id: record.id,
...
export function mapReviewWithUser(
  record: RepositoryReviewWithUserRecord,
): Review & { user: UserProfile | undefined } {
  return {
    ...mapReview(record),

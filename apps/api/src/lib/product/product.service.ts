...
export class ProductService {
  ...
  async listByBusiness(businessId: string) {
    const business =
      await this.businessRepository.findBusinessAccessById(businessId);
    if (!business) {
      return [];
    }
  ...
  async listManaged(
  ...
  ): Promise<ManagementListResult<ManagedProductListItem>> {
    ...
    const includeAllBusinesses =
      currentUser.role === 'ADMIN' ||
      currentUser.role === 'SUPERADMIN' ||
      currentUser.role === 'GLOBALADMIN';
    const result = await this.repository.listManaged(
      input,
      currentUser.id,
      includeAllBusinesses,
    );
  ...
  async exportManagedCsv(input: ListManagedProductsInput) {
    ...
    const includeAllBusinesses =
      ...
      currentUser.role === 'SUPERADMIN' ||
      currentUser.role === 'GLOBALADMIN';
    const products = await this.repository.listManagedForExport(
      input,
      currentUser.id,
      includeAllBusinesses,
    );
  ...
  async importManaged(input: ImportManagedProductsInput) {
    ...
    const createdProducts = await this.repository.createMany(
      input.items.map((item) =>
        this.toCreateProductInput(item.product, input.businessId),
      ),
    );
  ...
  async update(input: UpdateProductInput) {
    ...
    const updatedProduct = await this.repository.update(input.productId, {
      ...
      type: nextType,
      configurationSummary:
        nextType === 'configurable' ? (nextConfigurationSummary ?? null) : null,
  ...
  private async validateManagedImport(input: ImportManagedProductsInput) {
    ...
    const errors: string[] = [];
    const business = await this.businessRepository.findBusinessAccessById(
      input.businessId,
    );
    ...
    if (
      ...
      input.items.forEach((row) => {
        errors.push(
          buildImportValidationMessage(
            row.rowNumber,
            'No tienes acceso para importar productos en este negocio.',
          ),
        );
      });
    ...
    let featuredCount = await this.repository.countFeaturedByBusiness(
      input.businessId,
    );
    ...
    for (const item of input.items) {
      if (
        ...
        errors.push(
          buildImportValidationMessage(
            item.rowNumber,
            'FREE_TRIAL solo permite productos destacados.',
          ),
        );
        continue;
      }
  ...
  private toCreateProductInput(
    product: ImportManagedProductDraft,
    businessId: string,
  ): CreateProductInput {
    return {
      businessId,
  ...
  private assertValidProductShape(
    type: ProductType,
    configurationSummary: string | undefined,
    ...
    if (type === 'configurable') {
      if (!configurationSummary) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message:
            'Los productos configurables requieren un resumen breve visible en catálogo.',
        });
      }
  ...
  private ensureBusinessCanBeManaged(
    business:
      | Pick<RepositoryBusinessAccessRecord, 'ownerId' | 'managers'>
      | RepositoryProductWithBusinessRecord['business'],
  ) {
    if (!this.currentUser) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
    ...
    if (
      !canManageBusiness(this.currentUser, {
        ownerId: business.ownerId,
        managers: business.managers.map((manager) =>
          typeof manager === 'string' ? manager : manager.userId,
        ),
        memberships:
          'memberships' in business ? business.memberships : undefined,
      })
    ) {
      throw new TRPCError({
        code: 'FORBIDDEN',
  ...
  private async ensureFeaturedRulesForCreate(
    business: RepositoryBusinessAccessRecord,
    isFeatured: boolean,
  ) {
    if (business.subscriptionType === 'FREE_TRIAL' && !isFeatured) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'FREE_TRIAL solo permite productos destacados.',
      });
    }

    if (business.subscriptionType === 'FREE_TRIAL' && isFeatured) {
      const featuredCount = await this.repository.countFeaturedByBusiness(
        business.id,
      );
      if (featuredCount >= 5) {
        throw new TRPCError({
  ...
  private async ensureFeaturedRulesForUpdate(
    product: RepositoryProductWithBusinessRecord,
    ...
    if (
      product.business.subscriptionType === 'FREE_TRIAL' &&
      nextIsFeatured &&
      !product.isFeatured
    ) {
      const featuredCount = await this.repository.countFeaturedByBusiness(
  ...
  private applyVisibility<T extends { isFeatured: boolean }>(
    products: T[],
    subscriptionType: RepositoryBusinessAccessRecord['subscriptionType'],
  ) {

...
function createProductWithBusinessRecord(
  overrides: Partial<RepositoryProductWithBusinessRecord> = {},
): RepositoryProductWithBusinessRecord {
...
describe('ProductService', () => {
  ...
  it('listByBusiness returns mapped Prisma-backed products', async () => {
    ...
    businessRepository.findBusinessAccessById.mockResolvedValue(
      createBusinessAccess({ subscriptionType: 'PREMIUM_PLUS' }),
    );
  ...
  it('prevents business managers from creating products for their assigned business', async () => {
    ...
    await expect(
      service.create({
        ...
        description:
          'Descripción suficientemente larga para creación por manager.',
  ...
  it('update persists product correctly for an authorized business user', async () => {
    ...
    repository.update.mockResolvedValue(
      createProductRecord({ name: 'Producto ajustado', price: 350 }),
    );
    ...
    expect(repository.update).toHaveBeenCalledWith(
      'prod-1',
      expect.objectContaining({ name: 'Producto ajustado', price: 350 }),
    );
    expect(result).toMatchObject({
      id: 'prod-1',
      name: 'Producto ajustado',
  ...
  it('handles non-existent business and product cases correctly', async () => {
    const { service, businessRepository, repository } =
      createService(ownerUser);
    ...
    await expect(
      service.update({ productId: 'missing-product', name: 'Nada' }),
    ).rejects.toMatchObject({
      code: 'NOT_FOUND',
  ...
  it('enforces FREE_TRIAL featured-product limits on create', async () => {
    const { service, businessRepository, repository } =
      createService(ownerUser);
    businessRepository.findBusinessAccessById.mockResolvedValue(
      createBusinessAccess({ subscriptionType: 'FREE_TRIAL' }),
    );
    ...
    await expect(
      service.create({
        ...
        description:
          'Descripción suficientemente larga para el límite FREE_TRIAL.',
        images: ['https://example.com/product.jpg'],
        type: 'simple',
        price: 100,
        isFeatured: true,
        ...
        items: [
          {
            ...
            product: {
              name: 'No destacado',
              description:
                'Descripción suficientemente larga para validación de importación.',
  ...
  it('exportManagedCsv preserves admin export access across businesses', async () => {
    ...
    expect(repository.listManagedForExport).toHaveBeenCalledWith(
      expect.anything(),
      'admin-luis',
      true,
    );
  ...
  it('preserves the key contract shape for product responses', async () => {
    ...
    repository.listByBusiness.mockResolvedValue([
      createProductRecord({ price: null }),
    ]);

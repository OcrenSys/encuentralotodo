...
export interface ProductRepositoryPort {
  ...
  ): Promise<RepositoryManagedProductListResult>;
  listManagedForExport(
    input: ListManagedProductsInput,
    actorId: string | null,
    includeAllBusinesses: boolean,
  ): Promise<RepositoryManagedProductListRecord[]>;
  ...
  findByIdWithBusiness(
    productId: string,
  ): Promise<RepositoryProductWithBusinessRecord | null>;
  create(input: CreateProductInput): Promise<RepositoryProductRecord>;
  update(
    productId: string,
    input: Omit<UpdateProductInput, 'productId'>,
  ): Promise<RepositoryProductRecord | null>;
...
function mapProductWithBusinessRecord(
  record: any,
  ...
  return {
    ...
    business: {
      ...
      managers: (record.business.managers ?? []).map((manager: any) => ({
...
function mapManagedProductListRecord(
  record: any,
): RepositoryManagedProductListRecord {
  return {
    ...mapProductRecord(record),
    business: {
      id: record.business.id,
...
export class ProductRepository implements ProductRepositoryPort {
  ...
  async listManaged(
    ...
    const where = buildManagedProductsWhere(
      input,
      actorId,
  ...
  async listManagedForExport(
    input: ListManagedProductsInput,
    actorId: string | null,
    includeAllBusinesses: boolean,
  ) {
    const filters = buildManagedProductsWhere(
      input,
      actorId,
      includeAllBusinesses,
    );
          ...
          data: {
            ...
            configurationSummary:
              item.type === 'configurable'
                ? (item.configurationSummary ?? null)
                : null,
      ...
      data: {
        ...
        configurationSummary:
          input.type === 'configurable'
            ? (input.configurationSummary ?? null)
            : null,

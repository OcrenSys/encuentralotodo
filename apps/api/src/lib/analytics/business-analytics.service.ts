...
export class BusinessAnalyticsService {
  ...
  constructor({
    ...
    businessRepository,
    currentUser,
  }: BusinessAnalyticsServiceDependencies) {
  ...
  async getOverview(
    ...
    const sincePeriod =
      period === '7D' ? since7Days : period === '30D' ? since30Days : undefined;
    ...
    const reviewStrength = resolveReviewStrength(
    ...
    const upsellCandidateReasons = [
      ...
      overview.totalProducts > 0 && overview.totalPromotions === 0
        ? 'Has products but no promotions'
        : null,
      overview.totalReviews >= 5 && (overview.averageRating ?? 0) >= 4.2
  ...
  private ensureBusinessCanBeManaged(business: {
    ownerId: string;
    managers: string[];

...
export interface PlatformAnalyticsRepositoryPort {
  getSummary(input: {
    now: Date;
    since30Days: Date;
  }): Promise<RepositoryPlatformAnalyticsSummaryRecord>;
  listBusinessActivity(input: {
    now: Date;
  }): Promise<RepositoryPlatformBusinessActivityRecord[]>;
  ...
  async listBusinessActivity(input: { now: Date }) {
    ...
    return records.map(
      (record: {
        ...
        leadCreatedAt: record.leads.map(
          (lead: { createdAt: Date }) => lead.createdAt,
        ),
        productCount: record.products.length,
        activePromotionCount: record.promotions.filter(
          (promotion: { endDate: Date }) => promotion.endDate >= input.now,
        ).length,
        totalPromotionCount: record.promotions.length,
        reviewRatings: record.reviews.map(

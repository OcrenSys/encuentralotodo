...
function createBusinessAccessRecord(
...
function createOverviewRecord(
  overrides: Partial<RepositoryBusinessAnalyticsOverviewRecord> = {},
): RepositoryBusinessAnalyticsOverviewRecord {
  return {
...
function createService(
  currentUser: ReturnType<typeof createCurrentUser> | null,
) {
...
describe('BusinessAnalyticsService', () => {
  it('returns persisted overview metrics, trend points and monetization signals', async () => {
    ...
    expect(result).toMatchObject({
      ...
      recentLeads: [
        expect.objectContaining({ id: 'lead-1', businessId: 'biz-casa-norte' }),
      ],
      recentPromotions: [
    ...
    expect(
      result.leadTrend.some(
        (point) => point.date === '2026-03-25' && point.count === 2,
      ),
    ).toBe(true);
  });
  ...
  it('scopes access to owner or manager and rejects unrelated users', async () => {
    ...
    await expect(
      service.getOverview({ businessId: 'biz-casa-norte' }),

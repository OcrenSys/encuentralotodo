...
export interface BusinessAnalyticsRepositoryPort {
  getOverview(input: {
    ...
    since7Days: Date;
    since30Days: Date;
  }): Promise<RepositoryBusinessAnalyticsOverviewRecord | null>;
  listLeadTimestamps(input: {
    businessId: string;
    since?: Date;
  }): Promise<Date[]>;
}
...
function mapRecentLeadRecord(
  record: any,
): RepositoryBusinessAnalyticsRecentLeadRecord {
  return {
    id: record.id,
    name: record.name,
...
function mapRecentPromotionRecord(
  record: any,
): RepositoryBusinessAnalyticsRecentPromotionRecord {
  return {
    id: record.id,
    title: record.title,
    businessId: record.businessId,

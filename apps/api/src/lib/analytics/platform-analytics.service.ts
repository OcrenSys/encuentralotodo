...
export class PlatformAnalyticsService {
  ...
  constructor({
    repository,
    currentUser,
  }: PlatformAnalyticsServiceDependencies) {
    this.repository = repository;
    this.currentUser = currentUser;
  }

  async getOverview(
    input: GetPlatformAnalyticsInput = {},
    ...
    const enrichedActivity = businessActivity.map((business) => {
      ...
      const leadCountLast30Days = business.leadCreatedAt.filter(
        (createdAt) => createdAt >= since30Days,
      ).length;
      const reviewCount = business.reviewRatings.length;
      const averageRating =
        reviewCount > 0
          ? business.reviewRatings.reduce((sum, rating) => sum + rating, 0) /
            reviewCount
          : null;
      ...
      const highActivity = isHighActivityBusiness(
        leadCountLast30Days,
        engagementScore,
      );
      const leadVolumeBucket = resolveLeadVolumeBucket(leadCountLast30Days);
      const promotionUsageLevel = resolvePromotionUsageLevel(
        business.activePromotionCount,
      );
      const reviewStrength = resolveReviewStrength(reviewCount, averageRating);
      const indicators = [
        highActivity ? 'High lead activity' : null,
        leadCountLast30Days === 0
          ? 'No recent lead activity in last 30 days'
          : null,
        business.productCount > 0 && business.activePromotionCount === 0
          ? 'Products listed but no active promotions'
          : null,
      ...
      const monetizationReasons = [
        highActivity
          ? 'Candidate for premium analytics or lead-based plans'
          : null,
        business.productCount > 0 && business.activePromotionCount === 0
          ? 'Could benefit from boosts or promotion add-ons'
          : null,
        reviewStrength === 'STRONG' && business.status === 'APPROVED'
          ? 'Could support featured listing experiments'
          : null,
    ...
    const engagementIndicators = enrichedActivity
      ...
      .sort(
        (left, right) =>
          right.engagementScore - left.engagementScore ||
          right.leadCountLast30Days - left.leadCountLast30Days,
      )
      .slice(0, 10)
      .map((business) => ({
        businessId: business.businessId,
    ...
    const monetizationCandidates = enrichedActivity
      .filter(
        (business) =>
          business.monetizationReasons.length > 0 &&

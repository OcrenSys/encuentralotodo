...
export function resolvePromotionUsageLevel(
  activePromotionCount: number,
): PromotionUsageLevel {
...
export function resolveReviewStrength(
  reviewCount: number,
  averageRating: number | null,
): ReviewStrengthLevel {
...
export function isHighActivityBusiness(
  leadCountLast30Days: number,
...
export function resolveTrendRange(
  ...
  if (period === '7D') {
    return {
      start: normalizeUtcDate(subtractDays(now, 6)),
      end: normalizeUtcDate(now),
    };
  }

  if (period === '30D') {
    return {
      start: normalizeUtcDate(subtractDays(now, 29)),
      end: normalizeUtcDate(now),
    };
  }

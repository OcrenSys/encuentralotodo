...
function createSummaryRecord(
  overrides: Partial<RepositoryPlatformAnalyticsSummaryRecord> = {},
): RepositoryPlatformAnalyticsSummaryRecord {
  return {
...
function createBusinessActivityRecord(
  overrides: Partial<RepositoryPlatformBusinessActivityRecord> = {},
): RepositoryPlatformBusinessActivityRecord {
  return {
  ...
  it('builds platform summary, leaderboard and monetization indicators for admins', async () => {
    ...
    repository.listBusinessActivity.mockResolvedValue([
      ...
      createBusinessActivityRecord({
        ...
        leadCreatedAt: Array.from(
          { length: 12 },
          (_, index) =>
            new Date(
              `2026-03-${String(index + 10).padStart(2, '0')}T10:00:00.000Z`,
            ),
        ),

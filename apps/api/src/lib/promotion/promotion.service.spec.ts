...
function createPromotionWithBusinessRecord(
  overrides: Partial<RepositoryPromotionWithBusinessRecord> = {},
): RepositoryPromotionWithBusinessRecord {
...
describe('PromotionService', () => {
  ...
  it('update persists promotion correctly for an owner', async () => {
    ...
    repository.findByIdWithBusiness.mockResolvedValue(
      createPromotionWithBusinessRecord(),
    );
    ...
    expect(repository.update).toHaveBeenCalledWith(
      'promo-1',
      expect.objectContaining({ endDate: '2026-04-30T12:00:00.000Z' }),
    );
    expect(result).toMatchObject({
      id: 'promo-1',
      title: 'Oferta extendida',
  ...
  it('rejects delete without owner-admin access', async () => {
    ...
    await expect(
      service.delete({ promotionId: 'promo-1' }),
    ).rejects.toMatchObject({
      code: 'FORBIDDEN',
      message: 'Only the owner or a platform admin can manage promotions.',
    });
  ...
  it('rejects invalid date ranges on create', async () => {
    ...
    await expect(
      service.create({
        ...
        description:
          'La campaña no debe permitir una fecha final anterior a la inicial.',
        type: 'DISCOUNT',
  ...
  it('returns not found when updating a missing promotion', async () => {
    ...
    await expect(
      service.update({ promotionId: 'missing-promo', title: 'Nada' }),
    ).rejects.toMatchObject({
      code: 'NOT_FOUND',
  ...
  it('returns not found when deleting a missing promotion', async () => {
    ...
    await expect(
      service.delete({ promotionId: 'missing-promo' }),
    ).rejects.toMatchObject({
      code: 'NOT_FOUND',
      message: 'Promotion not found.',
    });

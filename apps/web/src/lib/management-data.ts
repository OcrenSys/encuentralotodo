...
const taskFixtures = {
  SUPERADMIN: [
    {
      id: 'task-sa-1',
      title: 'Revisar negocios pendientes en cola',
      state: 'Today',
      owner: 'Ops',
    },
    {
      id: 'task-sa-2',
      title: 'Validar consistencia de planes premium',
      state: 'Queued',
      owner: 'Billing',
    },
    {
      id: 'task-sa-3',
      title: 'Cerrar reporte de salud de catálogo',
      state: 'Blocked',
      owner: 'Data',
    },
  ],
  OWNER: [
    {
      id: 'task-own-1',
      title: 'Actualizar branding de Casa Norte Market',
      state: 'Today',
      owner: 'Sofia',
    },
    {
      id: 'task-own-2',
      title: 'Ajustar promociones antes del cierre',
      state: 'Queued',
      owner: 'Marketing',
    },
    {
      id: 'task-own-3',
      title: 'Responder leads corporativos',
      state: 'Today',
      owner: 'Ventas',
    },
  ],
  MANAGER: [
    {
      id: 'task-mgr-1',
      title: 'Revisar productos destacados de la semana',
      state: 'Today',
      owner: 'Carlos',
    },
    {
      id: 'task-mgr-2',
      title: 'Responder nuevos mensajes de promo',
      state: 'Queued',
      owner: 'Carlos',
    },
    {
      id: 'task-mgr-3',
      title: 'Confirmar horario extendido del negocio',
      state: 'Blocked',
      owner: 'Owner',
    },
  ],
...
function sortByRecent(
  left: { lastUpdated: string },
  right: { lastUpdated: string },
) {
  return (
...
export function useManagementData() {
  ...
  const canViewPlatformData = isMockMode
    ? roleView === 'SUPERADMIN'
    : isPlatformAdmin;
  ...
  const pendingBusinessesQuery = trpc.admin.pendingBusinesses.useQuery(
    undefined,
    {
      enabled: canViewPlatformData,
      retry: false,
  ...
  const managedBusinesses = useMemo(
    () =>
      ((managedBusinessesQuery.data ?? []) as BusinessDetails[]).sort(
        sortByRecent,
      ),
  ...
  const allBusinesses = useMemo(() => {
    if (isMockMode) {
      return ((businessQuery.data ?? []) as BusinessSummary[]).sort(
        sortByRecent,
      );
    }
  ...
  const accessibleBusinesses = useMemo(() => {
    ...
    if (roleView === 'OWNER') {
      return allBusinesses.filter(
        (business) => business.ownerId === roleProfile.userId,
      );
    }

    return allBusinesses.filter((business) =>
      business.managers.includes(roleProfile.userId),
    );
  }, [
    allBusinesses,
    isMockMode,
  ...
  const ownsManagedBusinesses = isMockMode
    ? roleView === 'OWNER'
    : managedBusinesses.some(
        (business) => business.owner?.id === backendUser?.id,
      );
  const shouldQueryBusinessAnalytics =
    Boolean(primaryBusiness) && !canViewPlatformData;
    ...
    return promotionsSource
      .filter(
        (promotion) =>
          !isMockMode ||
          roleView === 'SUPERADMIN' ||
          visibleIds.has(promotion.businessId),
      )
      .map((promotion) => ({
        ...promotion,
        businessName:
          allBusinesses.find((business) => business.id === promotion.businessId)
            ?.name ?? 'Negocio',
      }));
  }, [
  ...
  const teamMembers = useMemo<TeamMemberRecord[]>(() => {
        ...
        business.managersDetailed.forEach((manager) => {
          ...
          teamById.set(manager.id, {
            ...
            businessIds: existingManager
              ? Array.from(
                  new Set([...existingManager.businessIds, business.id]),
                )
              : [business.id],
          });
        });
    ...
    return marketplaceSeed.users
      ...
      .map((user) => ({
        ...
        membershipRole: accessibleBusinesses.some(
  ...
  const recentActivity = useMemo<ActivityRecord[]>(() => {
    if (canViewPlatformData && platformAnalyticsQuery.data) {
      const leaderboardActivity =
        platformAnalyticsQuery.data.businessActivityLeaderboard.map(
          (business) => ({
      ...
      const candidateActivity =
        platformAnalyticsQuery.data.monetizationCandidates.map((business) => ({
          id: `candidate-${business.businessId}`,
          title: `${business.businessName} listo para upsell`,
          detail:
            business.reasons[0] ??
            'Actividad suficiente para evaluación comercial.',
    ...
    if (isMockMode && businessAnalyticsQuery.data) {
      const leadActivity = businessAnalyticsQuery.data.recentLeads.map(
        (lead) => ({
          id: `lead-${lead.id}`,
          title: lead.name,
      ...
      const promotionActivity =
        businessAnalyticsQuery.data.recentPromotions.map((promotion) => ({
    ...
    const promotionActivity = managedPromotions
      .slice(0, 3)
      .map((promotion) => ({
        id: `promotion-${promotion.id}`,
        title: promotion.title,
    ...
    const adminActivity = (pendingBusinessesQuery.data ?? [])
      .slice(0, 2)
      .map((business) => ({
    ...
    return [
      ...businessActivity,
      ...promotionActivity,
      ...(canViewPlatformData ? adminActivity : []),
    ].slice(0, 6);
  }, [
    accessibleBusinesses,
  ...
  const platformHealth = useMemo(() => {
    if (platformAnalyticsQuery.data) {
      const pendingCount =
        platformAnalyticsQuery.data.summary.pendingBusinesses;
      if (pendingCount === 0) {
        return 'Stable';
      }
      if (pendingCount < 3) {

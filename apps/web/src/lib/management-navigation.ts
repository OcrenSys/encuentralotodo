...
export const navigationItems: NavigationItem[] = [
  ...
  },
  {
    ...
    description:
      'Resumen operativo, indicadores clave y accesos rápidos según el rol.',
    demoRoles: ['SUPERADMIN', 'OWNER', 'MANAGER'],
    group: 'operation',
...
export function getNavigationGroupsForAccess(context: NavigationAccessContext) {
...
export function getMobileNavigationForAccess(context: NavigationAccessContext) {
  return getNavigationForAccess(context)
    ...
    .sort(
      (left, right) =>
        (left.mobilePriority ?? 99) - (right.mobilePriority ?? 99),
    )
    .slice(0, 5);
}

...
function hasCompleteFirebasePublicConfig(
...
export function getPublicAuthProvider() {
  return resolvePublicRuntimeEnv()
    .NEXT_PUBLIC_AUTH_PROVIDER as PublicAuthProvider;
}
...
export function subscribeToAuthTokenChanges(
  listener: (user: FrontendAuthUser | null) => void,
) {
  if (!isFirebaseAuthEnabled()) {
    listener(null);
    return () => undefined;
...
export async function signInWithFirebaseEmail(input: {
  email: string;
  password: string;
}) {
  await ensurePersistence();
